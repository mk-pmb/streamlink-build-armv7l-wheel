...
function makewheel_init () {
  ...
  makewheel_fallible_pip_stuff &> >(
    ts | tee -- "$ORIG_WD/tmp.$FUNCNAME.log.txt")
  local RV=$?
  cd -- "$ORIG_WD" || return $?
...
function find_files () {
  ...
  FIND=(
    ...
    -name .git -prune ,
    -name .svn -prune ,
    -wholename '*/node_modules*/node_modules' -prune ,
    -type f
    )
  "${FIND[@]}" |& sort --version-sort | nl -ba | tee -- "$1" | tail
}
...
function makewheel_fallible_pip_stuff () {
  ...
  # :TODO: Where does the disabling option come from?
  #   Maybe because of --isolated?
